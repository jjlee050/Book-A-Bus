...
public class MainPageActivity extends ActionBarActivity implements TaskListener, LocationListener {
    ...
    public void onPostExecute(Class<?> cname, Object result) {
       ...
       /* for(int i = 0; i < busStopList.size(); i++)
        ...
        args.putString("lat", currentLat);
        args.putString("lon", currentLon);
        Parcelable wrapped = Parcels.wrap( busArrList);
        args.putParcelable("busArrList", wrapped);
        frag.setArguments(args);
        getFragmentManager().beginTransaction().remove(currentLocationfragment).commit();
